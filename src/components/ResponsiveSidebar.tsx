...
const ResponsiveSidebar = () => {
  ...
  const legalItems = [
    ...
    { path: '/returns', icon: RefreshCw, label: 'Returns Policy' },
    { path: '/shipping', icon: Truck, label: 'Shipping Policy' },
    { path: '/cookies', icon: Cookie, label: 'Cookies' },
    ...(user?.email === 'aslam@gmail.com' ? [{ path: '/admin-support', icon: Shield, label: 'Admin Panel' }] : []),
  ];
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        fixed lg:relative top-0 left-0 h-full w-64 bg-white border-r border-gray-200 flex flex-col z-50 transform transition-transform duration-300 ease-in-out
